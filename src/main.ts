...
function makeClankStringifier(quoteStringFunction: typeof quoteString) {
  ...
  function stringifyClankInternal(data: Clankable, topLevel = true): string {
    ...
    if (typeof data === 'string') {
      if (needsQuoting(data)) {
        return quoteStringFunction(data)
      }
      return data
    }
    ...
    if (data instanceof Map) {
      ...
      for (const [key, value] of data) {
        const serializedKey = needsQuoting(key) ? quoteStringFunction(key) : key
  ...
  return {
    needsQuoting,
    quoteString: quoteStringFunction,
    stringifyClank,
  }
}
const {needsQuoting,
  stringifyClank} = makeClankStringifier(quoteString)

export {needsQuoting, quoteString}

export const stringifyClankRuntimeSource = `const {stringifyClank} = (${String(makeClankStringifier)})((${String(quoteString)}))`
